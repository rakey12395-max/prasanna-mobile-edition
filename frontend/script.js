let API_KEY = localStorage.getItem('jarvrvis_key');
if(!API_KEY){
  API_KEY = prompt('Enter your Gemini API Key:');
  if(API_KEY) localStorage.setItem('jarvrvis_key', API_KEY);
}// // ===== 2. SMART MODELS (ఒకటిటిfail అయితేతేnext auto try) =====
const MODELS = ["gemini-3.6-flash", "gemini-flash-latest"];
const chat=document.getElementById('chat');
const input=document.getElementById('msg');
const micBtn=document.getElementById('mic-btn');// // ===== 3. GEMINI BRAIN (auto-fallback) =====
...
async function askGemini(p){add('J.A.R.V.I.S: Thinking...','ai');
                            try{
                              ...
                              chat.lastChild.innerTeText='J.A.R.V.I.S: '+reply;
                              speak(reply); // reply వచ్చి చ్చి న వెవెంటనేనేVOICE
                            }catch(e){
                              chat.lastChild.innerTeText='J.A.R.V.I.S: ERROR - '+e.message;
                            }
...
rec.onend=()=>{micBtn.innerTeText='🎙';};// // ===== 5. TEXT-T-TO-SPEECH (మామాట్లాటట్లాడటం) =====
let voices=[];
