...
async function askGemini(p){add('P.R.A.S.A.N.N.A: Thinking...','ai');
                            try{
                              const reply=await callGemini(p);
                              chat.lastChild.innerText='P.R.A.S.A.N.N.A: '+reply;
                              speak(reply); // reply వచ్చి చ్చి న వెవెంటనేనేVOICE
                            }catch(e){
                              chat.lastChild.innerText='P.R.A.S.A.N.N.A: ERROR - '+e.message;
                            }
...
rec.onend=()=>{micBtn.innerText='🎙';};// // ===== 5. TEXT-T-TO-SPEECH (మామాట్లాటట్లాడటం) =====
let voices=[];
